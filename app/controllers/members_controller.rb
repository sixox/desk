class MembersController < ApplicationController
  before_action :authenticate_user!, only: %i[
    show
    edit_description update_description
  ...
  def show
    ...
    @kpi_lists =
      if @user.procurement? && @user.is_manager?
  ...
  def build_month_context_for_user!(user_id, shamsi_month)
    ...
    @profile_archive = SalaryArchive.includes(:days).find_by(
      user_id: user_id,
    ...
    # ✅ NEW: Global remote days for this shamsi_month (applies to ALL users)
  ...
  # ✅ NEW: read from shamsi_month_remote_days table
